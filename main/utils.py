...
def find_mean_gap(first_column, second_column):
  fc_mean_height, fc_mean_width, sc_mean_height, sc_mean_width = mean_cell_size(first_column, second_column)
  gaps_first = []
  gaps_second = []
...
def restore_missed_cells(column_first, column_second, min_height, max_height, table, mode):
  fc_mean_height, fc_mean_width, sc_mean_height, sc_mean_width = mean_cell_size(column_first, column_second)
  ...
  if mode == 'second' or mode == 'both':
    for iter in range(restored_second.shape[0]-1):
      if abs(restored_second[iter][3] - restored_second[iter+1][1]) > sc_mean_height/3:
        phantom_box = [restored_second[iter][0], restored_second[iter][3]-gap_second, restored_second[iter][0] + sc_mean_width, restored_second[iter][3] + sc_mean_height -gap_second]
        if phantom_box[1] >= table[1] and phantom_box[2]<= table[2] and phantom_box[3] <=table[3] + sc_mean_height/3 and center_condition_of_missing_cell(restored_first, (phantom_box[3]-phantom_box[1])/2+phantom_box[1]):
          restored_second = np.insert(restored_second, iter+1, phantom_box, axis=0)
          restored_first, restored_second = restore_missed_cells(restored_first, restored_second, min_height, max_height, table, 'second')
          #break
...
def postfilterByCentresZero(first_column, second_column):
  ...
  zero_cell_center = (first_column[0][3] - first_column[0][1])/2 + first_column[0][1]
...
def postfilterByCentres(first_column, second_column):
  ...
  for iter in range(length):
    cell_center = (second_column[iter][3] - second_column[iter][1])/2 + second_column[iter][1]
    if not (cell_center > first_column[iter][1] and cell_center < first_column[iter][3]):
      _second_column = np.delete(_second_column,iter,axis=0)
      _first_column, _second_column = postfilterByCentres(_first_column, _second_column)
      break
  return _first_column, _second_column
...
def postSmoothSecColumn(first_column, second_column):
  ...
  gap_first, gap_second = find_mean_gap(first_column, second_column)
  cell_center = (first_column[lengthF][3] - first_column[lengthF][1])/2 + first_column[lengthF][1]
  if not (cell_center > second_column[lengthS][1] and cell_center < second_column[lengthS][3]):
    phantom_box = [_second_column[lengthS][0], _second_column[lengthS][3]-gap_second, _second_column[lengthS][0] + sc_mean_width, _second_column[lengthS][3] + sc_mean_height -gap_second]
